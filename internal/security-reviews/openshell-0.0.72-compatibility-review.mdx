...
It uses host networking and read-only Docker socket access, so hosts with glibc 2.39 or newer remain preferred for the reviewed OpenShell 0.0.72 pin.
...
- `regressionTest`: `src/lib/onboard/docker-driver-gateway-compat-container.test.ts` proves the explicit opt-in gates the fallback and covers container launch, the trust boundary, and the glibc decision.
- `removalCondition`: Remove the fallback when every supported Linux host meets OpenShell's native glibc 2.39-or-newer floor and the gateway-authentication and gateway-upgrade matrix passes for the release candidate commit without the flag.
...
- `regressionTest`: `test/installer-integration/install-openshell-version-check.test.ts` proves the development channel fails without `NEMOCLAW_ACCEPT_DEV_UNVERIFIED_INSTALL=1` and succeeds with it.
...
Every NemoClaw policy read-modify-write path, including preset merges and blueprint additions, starts from the base policy; at the time of this review, the then-supported and now-retired Shields snapshot-for-restore path used the same source:
...
The strict blueprint-addition schema does not author MCP endpoints; `nemoclaw <sandbox> mcp add` is the supported managed product path described in [Add an MCP Server](../../docs/manage-sandboxes/mcp-servers/add-an-mcp-server.mdx) and the [accepted architecture decision](https://github.com/NVIDIA/NemoClaw/issues/566#issuecomment-4847534784).
...
The stable `mcp-bridge` live lane isolates that upstream contract from NemoClaw's MCP implementation before the OpenClaw scenario performs any managed MCP mutation.
...
An exact `0.0.72` match runs the same full lifecycle, while any other development version records the expected fail-closed compatibility result and skips managed MCP mutation.
The full lifecycle applies a raw OpenShell `protocol: mcp` policy with `allowed_ips: [1.1.1.1]`, remaps the hostname to a reachable private runner address, sends a raw MCP `tools/list` request, requires an exact HTTP 403, verifies zero upstream requests without calling `nemoclaw mcp` or any agent adapter, and restores the exact base policy in `finally`.
...
- The stable MCP live lane proves raw OpenShell `allowed_ips` rebinding denial with an exact HTTP 403 and zero upstream requests, then restores the base policy.
- The development MCP lane runs that full proof only when its moving OpenShell build matches the reviewed `0.0.72` credential boundary; otherwise it records the expected fail-closed version mismatch without managed MCP mutation.
