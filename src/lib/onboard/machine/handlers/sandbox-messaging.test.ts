...
describe("hasMessagingCredentialDrift", () => {
  ...
  it("detects only an explicitly supplied replacement credential", async () => {
  ...
  it("ignores replacement credentials for disabled channels", async () => {
...
describe("reconcileReusedSandboxMessaging", () => {
  it("does not clear an equal recorded plan from a different authority", async () => {
    ...
    const result = await reconcileReusedSandboxMessaging(
  ...
  it("rejects a retired channel without changing a Ready sandbox plan (#9283)", async () => {
  ...
  it("keeps a still-configured channel in a reused sandbox selection (#9283)", async () => {
    const plan = discordPlan(hashCredential("previous-discord-token") ?? "");
  ...
  it("keeps a bridge channel whose gateway credential outlived the onboarding process (#10660)", async () => {
  ...
  it("rejects Ready sandbox reuse when a bridge credential is missing (#10660)", async () => {
    const plan = googlechatPlan();
    vi.stubEnv("GOOGLECHAT_SERVICE_ACCOUNT", "");
    const note = vi.fn();
  ...
  it("keeps a token channel whose provider still matches at the gateway (#10660)", async () => {
    const plan = discordPlan(hashCredential("previous-discord-token") ?? "");
    vi.stubEnv("DISCORD_BOT_TOKEN", "");
    const inspectGatewayCredential = vi.fn(() => ({ kind: "exact" as const }));
  ...
  it.each([
    ...
    "rejects Ready sandbox reuse when its Slack %s is missing (#10660)",
    async (_, missing) => {
  ...
  it("preserves Slack when a missing binding accompanies an indeterminate inspection (#10660)", async () => {
    const plan = slackPlan(
      hashCredential("previous-slack-bot-token") ?? "",
      hashCredential("previous-slack-app-token") ?? "",
    );
  ...
  it("keeps an in-sandbox QR channel in a reused sandbox selection (#9283)", async () => {
  ...
  it("removes every unsupported channel artifact from a reused plan", async () => {
    vi.stubEnv("TELEGRAM_BOT_TOKEN", "123456:registry-token");
    const result = await reconcileReusedSandboxMessaging(
      mixedChannelPlan(),
      { name: "openclaw" },
