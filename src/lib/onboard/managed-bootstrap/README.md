...
`launch` callback returns a validated Ready receipt with the materialized
...
validation, both the cleanup request and its result are bound to the sandbox ID.
...
0400 envelope and returns cleanup authority without quiescing, renaming,
...
rollback can retain the same phase only after it removes the replacement.
This path restores the original to its canonical name and keeps it
...
must prove the runtime is absent. Unknown runtime presence is a retryable
...
operation-scoped Podman command adapter to the rootless endpoint, captures
...
replacement and its private managed state volume while retaining the original. The journal records every mutation boundary before the next external
...
root-apply envelope, starts the replacement, and authenticates the
...
namespace, compacts that state to a commit receipt, and rejects rollback
...
The parser accepts the canonical schema-v1 manifest written before
...
identity cannot be proven, blocks create. A failure for another sandbox
...
record and any decision sidecar, reports its bootstrap, provider, sandbox,
...
3. If either runtime is present, or its presence cannot be proven, leave
...
4. If both runtimes are proven absent, still preserve the journal and its
   image-owned shared-state evidence. Record the absence proof on
...
bootstrap envelope,
bind shared-state authority to the attempt, publish an identity-bound
...
Pull-request and publication workflows build the images and exercise the
protected envelope, native bootstrap, production held-command renderer, and
all-agent hold contracts. Ordinary OpenShell Docker-driver onboarding selects the managed images for the shipped agents. Portable onboarding, non-managed agents,
