...
export type OpenClawBridgeHealthCommandRunner = (
  command: string,
  timeoutMs: number,
) => Promise<OpenClawBridgeHealthCommandResult | null | undefined>;
...
export function createOpenClawBridgeHealthHook(
  ...
  return () => {
    ...
    return (async () => {
      ...
      const configProbe = await execute(`cat ${OPENCLAW_CONFIG_FILE} 2>/dev/null || true`, 10000);
      ...
      const logProbe = await execute(
        `tail -n 400 ${OPENCLAW_GATEWAY_LOG_FILE} 2>/dev/null || true`,
        10000,
      );
