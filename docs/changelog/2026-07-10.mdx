...
- Sandbox base-image resolution now prefers release-matched image tags before mutable `:latest` for release installs and stale source checkouts, validates explicit base-image overrides, and fails closed when a requested image cannot be pulled or fails compatibility checks.
...
- Onboarding and recovery preserve intent across more interrupted flows.
  Live inference recovery is scoped to the sandbox, pending route reservations survive a not-ready recreate, re-onboarding probes registered extra providers, and compatible provider recovery is preserved through the rebuild handoff.
