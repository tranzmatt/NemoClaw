...
A direct container runtime that explicitly overrides the image user to `root` retains the existing `capsh` compatibility drop and `NEMOCLAW_REQUIRE_CAP_DROP=1` fail-closed option.
OpenShell-managed launches do not call that path.
