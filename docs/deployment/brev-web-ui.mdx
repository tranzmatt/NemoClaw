...
If you want to manage the remote host from a terminal, refer to [Deploy to a Remote GPU Instance](deploy-to-remote-gpu).
</Note>

## What This Flow Creates

The Brev web flow creates these resources:
...
<Note>
The **Show Other Providers** dropdown appears below the **NVIDIA Cloud** card.
Click it to expand the provider list.
The expanded list includes **OpenAI**, **Anthropic**, and **Google Gemini**.
For these providers, get the API key from the provider console before you create the agent.
</Note>
...
2. Click **GPUs** in the navigation bar.
