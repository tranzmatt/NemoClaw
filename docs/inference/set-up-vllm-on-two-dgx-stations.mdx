...
When both containers carry NemoClaw's complete schema-2 ownership labels, the default-port endpoint, and the matching historical launch contracts for this exact pair, NemoClaw authenticates them as prior managed state but does not reuse them.
