...
An operator must be able to identify the source, version, and immutable digest or checksum of an installed executable artifact.
...
| Policy | Make every network, filesystem, device, and capability contribution explicit, deny by default, reviewable, and reversible. | Tests prove the allowed scope, reject undeclared access, and restore the prior policy after removal or rollback. |
...
|---|---|---|
| #5998 | Managed agent package workflow with supporting lifecycle contributions. | It can add a narrow NemoClaw-managed add, list, status, remove, and rebuild path for an agent package. The package runs through the agent inside the sandbox. It does not create a NemoClaw plugin SDK, registry, marketplace, host extension point, or agent-native plugin API. |
