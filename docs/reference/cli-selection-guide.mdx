...
If the task inspects or changes the live OpenShell gateway, TUI, raw policy, port forwarding, inference route, or intentionally bypasses a NemoClaw file-transfer wrapper, use `openshell`.
...
- Manage arbitrary user-owned OpenShell forwards. For a NemoClaw-managed dashboard, API, or messaging forward, run `$$nemoclaw <name> recover` instead:
...
- Move files or run raw one-off commands when you intentionally want to bypass NemoClaw's sandbox registry and wrappers:
  ...
  The direct download command does not use NemoClaw's source validation, private staging, or destination publication safeguards.
...
Use `$$nemoclaw <name> download` for manual host downloads when you need control over source and destination paths.
Use `openshell sandbox upload` when you intentionally want the raw OpenShell upload path.
