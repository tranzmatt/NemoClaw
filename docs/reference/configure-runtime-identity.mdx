...
They prove that a child launched after token refresh retains the same stable credential handle while the protected resource receives the refreshed bearer.
The handle changes only after an authorization-binding epoch change, such as removing and restoring the credential key.
