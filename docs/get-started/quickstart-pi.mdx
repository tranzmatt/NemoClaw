...
<Warning>
Pi remains a release candidate until the final activation change adds it to the supported agent inventory and managed-image release cohort. Normal installations reject `--agent pi` before that activation. The qualification controls are not a user setup path.
</Warning>
...
```bash
nemoclaw my-pi exec --workdir /sandbox --no-tty -- \
  pi --no-approve --no-context-files --mode json --print --tools read \
    "Use the read tool to inspect README.md and report its title"
```

`--no-approve` ignores project-local executable resources but does not disable context files.
`--no-context-files` excludes `AGENTS.md` and `CLAUDE.md` from the prompt; omit it only when automation must accept project instructions.
`--tools read` limits that invocation to Pi's read tool.
JSON mode writes newline-delimited session events, including structured tool execution events and the final assistant message.
