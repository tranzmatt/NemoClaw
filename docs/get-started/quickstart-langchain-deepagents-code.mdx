...
Deep Agents Code state lives under `/sandbox/.deepagents`. NemoClaw snapshot and rebuild flows preserve the app state directory and skills when those paths exist. During managed re-onboarding, NemoClaw restores the allowlisted UI and thread preferences, validated native interpreter settings, and native `startup.mode` from `config.toml`. Freshly generated model routing, update settings, and provider metadata remain authoritative; unknown, privileged, invalid, and security-sensitive keys are dropped. User-authored `hooks.json` is also preserved, so review its executable commands before restoring a snapshot from another workspace. NemoClaw recreates the sandbox when its live `dcode identity` output is unreadable or does not match the selected provider and model, then records the selection only after the restored runtime passes the same check. Run `nemoclaw <sandbox-name> snapshot create` after active `dcode` tasks finish. For `langchain-deepagents-code` sandboxes, NemoClaw refuses backup when it detects an active `dcode` task or cannot verify that the state tree is idle. `.deepagents/.mcp.json` is the agent-native MCP source. Managed entries contain only URLs and OpenShell placeholders, and rebuild carries them through its bounded transaction handoff rather than reconstructing them from the host registry. Service credentials remain in OpenShell provider state. NemoClaw intentionally does not back up `.deepagents/.env`. If `.deepagents/.state/auth.json` contains upstream credentials, or `.deepagents/.state/chatgpt-auth.json` exists, the managed Deep Agents Code launch paths refuse to start until that credential state is removed. Before a managed Deep Agents Code rebuild changes the sandbox, NemoClaw selects its recorded OpenShell gateway, tests the recorded inference route through `https://inference.local`, and prepares the replacement from the current agent configuration, OpenShell policy and providers, model, and build inputs with a pinned base and fingerprinted context. Initial failures stop before backup. After backup, NemoClaw rechecks the target, route, and retained build inputs before changing MCP state, then checks again after MCP preparation and before stopping inference or deleting the old sandbox. If the final check fails, NemoClaw restores the previous source state and keeps the existing sandbox intact. Rebuild carries the complete current OpenShell policy forward, including Deep Agents Code `tavily`, custom entries, and trusted host-side edits. The recorded observability choice is preserved unless explicitly overridden.
