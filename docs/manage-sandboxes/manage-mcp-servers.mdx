...
For a trusted-private entry, restart replays the exact address pins recorded by `mcp add`.
...
The re-add performs a new DNS preflight and records the reviewed exact address set.
...
Exact legacy `generic` providers are accepted only for cleanup.
...
For an ordinary managed entry, NemoClaw cleans up the adapter, removes the exact owned policy, and then detaches its provider because OpenShell rejects detach while `credential_binding.provider` still references that provider.
