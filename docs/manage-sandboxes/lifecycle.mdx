...
<AgentOnly variant="hermes">
For Portable Hermes, text status reports the saved lifecycle phase and `Runtime and agent health: not probed`.
The saved phase does not establish live runtime or agent health.
</AgentOnly>
...
For non-Portable sandboxes, the debug command gathers system information, Docker state, sandbox logs, and sandbox status.

<AgentOnly variant="hermes">
For Portable Hermes, an explicitly selected sandbox produces a redacted offline lifecycle report, even when the registry and runtime are unavailable.
The archive contains only `portable-lifecycle.json`, with the report schema version, sandbox name, agent, saved lifecycle phase, and unprobed health values.
...
Refer to the [debug command reference](../../reference/commands#$$nemoclaw-debug) for selection and refusal behavior.
</AgentOnly>
