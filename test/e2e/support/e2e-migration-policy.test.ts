...
const RETIREMENT_DOC = path.join(E2E_SUITE_DIR, "docs", "RETIREMENT.md");
const FORBIDDEN_LEGACY_LEDGER = path.join(E2E_SUITE_DIR, "migration", "legacy-inventory.json");
...
describe("E2E migration tracking policy", () => {
  ...
  it("documents GitHub issues and PRs as the migration source of truth", () => {
    ...
    expect(docs).toContain("replacement E2E coverage");
    expect(docs).toContain("workflow contract test");
    expect(docs).toContain("source-shape checks");
    expect(docs).toContain("machine-checkable boundary is the source tree plus workflow tests");
    expect(docs).toContain("generated retired assertion inventories");
