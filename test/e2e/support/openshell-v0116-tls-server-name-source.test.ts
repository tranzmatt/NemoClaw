...
describe("OpenShell 0.0.116 TLS server-name boundary", () => {
  it("covers every local supervisor driver changed by the upstream security fix", () => {
    const checks = OPENSHELL_V0116_TLS_SERVER_NAME_SOURCES.flatMap(({ checks }) => checks);
  ...
  it("rejects a regression assertion that occurs before hostile-value injection", () => {
    const reviewedSource = OPENSHELL_V0116_TLS_SERVER_NAME_SOURCES[1]!;
  ...
  it("binds the live qualification target to the production supervisor map", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-v0116-supervisor-map-"));
  ...
  it("rejects removal that occurs before the user environment merge", () => {
    const reviewedSource = OPENSHELL_V0116_TLS_SERVER_NAME_SOURCES[0]!;
