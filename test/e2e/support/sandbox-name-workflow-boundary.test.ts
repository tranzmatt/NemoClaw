...
describe("live workflow sandbox name boundary", () => {
  it.each(
    WORKFLOW_PATHS,
  )("keeps every literal and matrix-generated sandbox identity canonical in %s (#8497)", (workflowPath) => {
  ...
  it("rejects overlong and unresolved optional-lane sandbox identities (#8497)", () => {
    ...
    workflow.jobs["bedrock-runtime-compatible-anthropic"]!.strategy!.matrix = {
      include: [
