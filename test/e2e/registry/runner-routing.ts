...
// `TargetEnvironment.platform` and `TargetDefinition.runnerRequirements`
// fields so that adding a target to the registry automatically resolves a
...
export function resolveRunnerForTarget(target: TargetDefinition): ResolvedRunner {
  const explicit = (target.runnerRequirements ?? []).find((req) => req.startsWith("runs-on:"));
  if (explicit) {
    const runner = explicit.slice("runs-on:".length).trim();
    if (!runner) {
      throw new Error(`Cannot resolve runner for target '${target.id}': empty runs-on override.`);
    }
  ...
  throw new Error(
    `Cannot resolve runner for target '${target.id}': no runs-on override and no default for platform '${platform ?? "<missing>"}'.`,
  );
}
