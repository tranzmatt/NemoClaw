...
/**
 * Live Vitest replacement for test/e2e/test-concurrent-gateway-ports.sh.
 *
 * Preserves the legacy real-system boundaries: two NemoClaw onboards on one
...
liveTest(
  ...
  async ({ artifacts, cleanup, host, sandbox, skip }) => {
    ...
    await artifacts.writeJson("scenario.json", {
      ...
      legacySource: "test/e2e/test-concurrent-gateway-ports.sh",
      contract: [
