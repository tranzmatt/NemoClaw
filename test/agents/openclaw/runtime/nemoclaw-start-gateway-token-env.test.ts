    ...
    it.each([
      ...
      ({ logMode }) => {
        ...
        const script = [
          ...
          '  process_executable="$(readlink "/proc/$GATEWAY_PID/exe" 2>/dev/null || true)"',
          '  case "$process_cmdline:$process_executable" in *nemoclaw-proc-credential-probe:*/node) ready=1; break ;; esac',
          "  sleep 0.01",
          "done",
