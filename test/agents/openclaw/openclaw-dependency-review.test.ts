...
describe("OpenClaw 2026.6.10 dependency review contract", () => {
  it("pins the active diagnostics Jaeger remediation to the shipped install path", () => {
    ...
    expect(review).toContain("test/agents/openclaw/openclaw-diagnostics-jaeger-runtime.test.ts");
  ...
  it("records the version-scoped transient remote MCP startup recovery patch (#7958)", () => {
    ...
    expect(review).toContain("test/agents/openclaw/openclaw-mcp-reliability-patch.test.ts");
  ...
  it("records the bounded MCP tool discovery timeout patch (#7957)", () => {
    ...
    expect(review).toContain("test/agents/openclaw/openclaw-mcp-tools-list-timeout-patch.test.ts");
  ...
  it("keeps advisor disposition evidence in the dependency review note", () => {
    ...
    expect(review).toContain("test/agents/openclaw/openclaw-real-patched-dist-harness.test.ts");
    expect(review).toContain("NEMOCLAW_REAL_OPENCLAW_DIST_HARNESS=1");
    ...
    expect(review).toContain("test/agents/openclaw/openclaw-issue-4434-diagnostics-patch.test.ts");
